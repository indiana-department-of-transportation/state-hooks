...
type HookReturn<T> = [Opt<T>, (state: Opt<T>) => void]
type StateHook<T> = {
  (): HookReturn<T>
}

const c = <T>(hook1: StateHook<T>, hook2: StateHook<T>): () => [Opt<T>, (state: Opt<T>) => void] => {
  return (): [Opt<T>, (state: Opt<T>) => void] => {
    ...
    const setter = useCallback((state: Opt<T>) => {
    ...
    const result: [Opt<T>, (state: Opt<T>) => void] = [value.current, setter];
    return result;
  };
};
